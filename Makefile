...
profile-cpu:
	go test -cpuprofile cpu.prof -run "^TestRTSPCameraPerformance$$" -bench github.com/erh/viamrtsp
	go tool pprof -top cpu.prof > cpu-profile.txt
	rm cpu.prof

profile-memory:
	go test -memprofile mem.prof -run "^TestRTSPCameraPerformance$$" -bench github.com/erh/viamrtsp
