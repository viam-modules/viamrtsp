...
func (p *videoStoreProxy) Fetch(ctx context.Context, r *videostore.FetchRequest) (*videostore.FetchResponse, error) {
	return p.vs.Fetch(ctx, r)
}

func (p *videoStoreProxy) Save(ctx context.Context, r *videostore.SaveRequest) (*videostore.SaveResponse, error) {
	return p.vs.Save(ctx, r)
}

func (p *videoStoreProxy) Close() {
	// p.workers.Stop()
	p.vs.Close()
}

func (p *videoStoreProxy) SDPParams(typ videostore.CodecType, au [][]byte) error {
...
func (p *videoStoreProxy) CloseSegmenter() error {
	// defer p.cancel()
	return p.vs.CloseSegmenter()
}

func (p *videoStoreProxy) SupportedCodecs() map[videostore.CodecType]struct{} {
	return p.vs.SupportedCodecs()
}

// func (p *videoStoreProxy) attemptReconnect(ctx context.Context, cam registry.ModuleCamera) {
// 	for {
// 		if ctx.Err() != nil {
// 			return
// 		}

// 		if err := cam.Register(p); err != nil {
// 			continue
// 		}
...
func New(ctx context.Context, deps resource.Dependencies, conf resource.Config, logger logging.Logger) (resource.Resource, error) {
	...
	var cam registry.ModuleCamera
	if newConf.Camera != nil {
		c, err := camera.FromDependencies(deps, *newConf.Camera)
		if err != nil {
			return nil, err
		}
		regCam, err := registry.Global.Camera(c.Name().String())
		if err != nil {
			return nil, err
		}
		...
		tmpVs := &videoStoreProxy{
			// ctx:     cancelCtx,
			// cancel:  cancel,
			vs: rtpVs,
			// workers: utils.NewBackgroundStoppableWorkers(),
		}
		if err := regCam.Register(tmpVs); err != nil {
			return nil, err
		}
		cam = regCam
		// tmpVs.workers.Add(func(ctx context.Context) {
		// 	for {
		// 		if ctx.Err() != nil {
		// 			return
		// 		}
		...
		// 			return
		// 		case <-cancelCtx.Done():
		// 			tmpVs.attemptReconnect(ctx, cam)
		// 		}
		// 	}
		// })
...
func (s *service) Close(_ context.Context) error {
	s.vs.Close()
	if s.cam != nil {
		p, ok := s.vs.(*videoStoreProxy)
		if !ok {
			return errors.New("couldn't find video store proxy")
		}
		if err := s.cam.DeRegister(p); err != nil {
			return err
		}
...
//	}
//}
//func (e *videoStoreMuxer) Save(cancelCtx context.Context, r *videostore.SaveRequest) (*videostore.SaveResponse, error) {
//	e.mu.Lock()
//	defer e.mu.Unlock()
//	if e.vs == nil {
//		return nil, errors.New("video-store uninitialized")
//	}
//	return e.vs.Save(cancelCtx, r)
//}
